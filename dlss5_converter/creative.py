...
def _orbit(p: float) -> tuple[float, float, float]:
    a = 2.0 * np.pi * p
    return np.cos(a), np.sin(a), 0.0            # gentle circular sway


def _push(p: float) -> tuple[float, float, float]:
    e = _loop(p)
    return 0.0, -0.15 * e, 0.9 * e              # dolly in, slight rise


def _float(p: float) -> tuple[float, float, float]:
    return 0.35 * np.sin(2 * np.pi * p), 0.5 * _loop(p) - 0.25, 0.2 * _loop(p)


def _sway(p: float) -> tuple[float, float, float]:
    return np.sin(2 * np.pi * p), 0.12 * np.cos(2 * np.pi * p), 0.0  # left-right
...
class CreativeSettings:
    ...
    preset: str = "Float"
    depth_intensity: float = 1.0          # 0..2, scales parallax + particle depth
    fog: float = 0.35                     # 0..1 density
    fog_color: tuple[int, int, int] = (150, 156, 168)  # RGB, cool haze
    embers: float = 0.0                   # 0..1 amount (warm rising motes)
    dust: float = 0.35                    # 0..1 amount (slow drifting motes)
...
class Renderer:
    ...
    def _warp(self, tx: float, ty: float, zoom: float, amp: float) -> np.ndarray:
        ...
        map_x, map_y = self._maps(tx, ty, zoom, amp)
    ...
    def _maps(self, tx: float, ty: float, zoom: float, amp: float
        ...
        z = OVERSCAN + zoom * 0.10
        shift = amp * self.near
        map_x = (self.cx + (self.X - self.cx) / z - tx * shift).astype(np.float32)
        map_y = (self.cy + (self.Y - self.cy) / z - ty * shift).astype(np.float32)
        return np.ascontiguousarray(map_x), np.ascontiguousarray(map_y)

    def _warp_scalar(self, field: np.ndarray, tx, ty, zoom, amp) -> np.ndarray:
        map_x, map_y = self._maps(tx, ty, zoom, amp)
    ...
    def _ensure_particles(self, kind: str, n: int) -> np.ndarray:
        """Lazily create a particle set: columns x, y, z(0..1 near), size, seed."""
        key = f"{kind}:{n}"
        if key not in self._particles:
            ...
            p[:, 2] = r.uniform(0.0, 1.0, n)          # depth (near=1)
            p[:, 3] = r.uniform(0.6, 1.8, n)          # base radius px
            p[:, 4] = r.uniform(0.0, 1.0, n)          # phase seed
    ...
    def _draw_particles(self, canvas: np.ndarray, kind: str, amount: float,
        ...
        if amount <= 0:
            return
        n = int(amount * (220 if kind == "dust" else 160))
        if n <= 0:
            return
        p = self._ensure_particles(kind, n)
        t = phase
        x = p[:, 0].copy(); y = p[:, 1].copy(); z = p[:, 2]
        # Drift: embers rise and waver, dust floats slowly sideways. Looping in t.
        if kind == "embers":
            y = (y - (t * self.h * 0.6)) % self.h
            x = (x + np.sin((t + p[:, 4]) * 2 * np.pi) * self.w * 0.02) % self.w
            col = np.array([1.0, 0.55, 0.18], np.float32); glow = True
        else:  # dust
            x = (x + (t * self.w * 0.10) + np.sin((t + p[:, 4]) * 2 * np.pi) * 6) % self.w
            y = (y + np.sin((t * 0.5 + p[:, 4]) * 2 * np.pi) * 4)
            col = np.array([0.9, 0.92, 1.0], np.float32); glow = False
        # Parallax: particles shift with the camera scaled by their own depth.
        sx = x + tx * amp * z
        sy = y + ty * amp * z
        # Occlusion: hide a particle if the scene at its spot is nearer than it.
        xi = np.clip(sx.astype(int), 0, self.w - 1)
        yi = np.clip(sy.astype(int), 0, self.h - 1)
        visible = self.near[yi, xi] <= z + 0.06
        twinkle = 0.6 + 0.4 * np.sin((t * 2 + p[:, 4]) * 2 * np.pi)
        # Scatter single bright pixels into a layer, then blur so each becomes a
        # soft round glow rather than a hard square. Near particles get a wider
        # blur so depth reads as focus falloff.
        layer = np.zeros((self.h, self.w), np.float32)
        idx = np.nonzero(visible)[0]
        if idx.size == 0:
            return
        xi2 = np.clip(sx[idx].astype(int), 0, self.w - 1)
        yi2 = np.clip(sy[idx].astype(int), 0, self.h - 1)
        strength = twinkle[idx] * (0.7 * z[idx] + 0.3)
        np.add.at(layer, (yi2, xi2), strength)
        sigma = max(1.0, self.w / 500.0) * (1.6 if glow else 1.1)
        layer = cv2.GaussianBlur(layer, (0, 0), sigma)
        # A blur spreads each impulse's energy over ~2*pi*sigma^2 pixels, so
        # multiply back by that to keep a particle's peak brightness independent
        # of the blur width. `amount` is already spent on particle count.
        peak = 1.2 if glow else 0.5
        gain = peak * 2.0 * np.pi * sigma * sigma
        canvas += layer[..., None] * col * gain

    # -- one frame -----------------------------------------------------------

    def frame(self, s: CreativeSettings, index: int) -> np.ndarray:
        phase = index / s.frames
        tx, ty, zoom = PRESETS[s.preset](phase)
        amp = AMP_PX * s.depth_intensity
        out = self._warp(tx, ty, zoom, amp)
        if s.fog > 0:
            fogf = self._warp_scalar(self.far, tx, ty, zoom, amp)
            fog = (fogf ** 1.4)[..., None] * (s.fog * 0.9)
            fc = np.array(s.fog_color, np.float32) / 255.0
            out = out * (1.0 - fog) + fc * fog
        self._draw_particles(out, "dust", s.dust, phase, tx, ty, amp)
        self._draw_particles(out, "embers", s.embers, phase, tx, ty, amp)
